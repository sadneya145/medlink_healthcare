...
def open_doctor_window():
    ...
    new_image_path = r"medlink_healthcare-main\medlink final\doctor_l.png"
...
def open_patient_window():
    ...
    new_image_path = r"medlink_healthcare-main\medlink final\eg.png"
...
def open_login_window(user_type):
    ...
    new_image_path = r"medlink_healthcare-main\medlink final\login2.png"
    new_pil_image = Image.open(new_image_path).resize((1000, 800), Image.LANCZOS)
    new_tk_image = ImageTk.PhotoImage(new_pil_image)
...
def open_signup_window(user_type):
    ...
    new_image_path = r"medlink_healthcare-main\medlink final\login1.png"
    new_pil_image = Image.open(new_image_path).resize((1000, 800), Image.LANCZOS)
    new_tk_image = ImageTk.PhotoImage(new_pil_image)
...
def open_main_d():
    ...
    # Doctor Buttons
    tk.Button(left_panel, text="My Profile", command=show_doctor_profile, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="View Appointments", command=open_doctor_profile, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Enter Fees", command=open_fee_entry_frame, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="View Patient Records", command=fetch_patient_records, width=20, height=2).pack(pady=10)
    ...
    image_path = r"medlink_healthcare-main\medlink final\d_home.jpg"
    img = Image.open(image_path)
    # img = img.resize((600, 800))  # Resize the image to fit the frame
...
def generate_receipt_pdf(consultation_fee ,lab_test_fee,medication_fee, current_patient_id,other_fees, doctor_name, appointment_details, total_amount):

    medical_records_directory = r"medlink_healthcare-main\medical_records"
    
    # Create the directory if it doesn't exist
    os.makedirs(medical_records_directory, exist_ok=True)

    # Create a PDF file
    pdf_filename = os.path.join(medical_records_directory, f"Receipt_Patient_{current_patient_name}.pdf")
    ...
    # Add patient details
    pdf_canvas.drawString(x_offset, y_offset, f"Patient ID: {current_patient_id}")
    y_offset -= line_height
    pdf_canvas.drawString(x_offset, y_offset, f"Patient Name: {current_patient_name}")
    y_offset -= line_height
...
def save_bill_to_database(current_patient_id, appointment_id, total_amount, additional_details):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute(insert_bill_query, (current_patient_id, appointment_id, total_amount, additional_details))
...
def open_main_p(root):
    ...
    # Buttons
    tk.Button(left_panel, text="My Profile", command=show_patient_profile, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Make Appointments", command=get_patient_name, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Show Appointments", command=open_patient_profile, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Chat with AI bot", command=launch_chatbot, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Bill Payments", command=show_pdf_files, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Medical Records", command=upload_medical_records, width=20, height=2).pack(pady=10)
    tk.Button(left_panel, text="Show Records", command=show_uploaded_records, width=20, height=2).pack(pady=10)
...
image_path = r"medlink_healthcare-main\medlink final\main.png"
