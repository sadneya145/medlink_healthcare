...
def make_video_call():
    # Here, you would implement the code to initiate the video call.
    # For demonstration purposes, let's assume it's already implemented.
    doctor_name, patient_name = get_doctor_and_patient_names()
    
    # Fetch meeting URL for patient
    patient_url_query = "SELECT MeetingLink FROM video_calls WHERE PatientID = (SELECT id FROM patient_info WHERE Name = %s)"
    patient_url_data = fetch_data(patient_url_query, (patient_name,))
    if patient_url_data:
        patient_meeting_url = patient_url_data[0][0]
    else:
        messagebox.showerror("Error", "Meeting URL not found for patient.")
        return

    # Fetch meeting URL for doctor
    doctor_url_query = "SELECT MeetingLink FROM video_calls WHERE DoctorID = (SELECT id FROM doctor_info WHERE Name = %s)"
    doctor_url_data = fetch_data(doctor_url_query, (doctor_name,))
    if doctor_url_data:
        doctor_meeting_url = doctor_url_data[0][0]
    else:
        messagebox.showerror("Error", "Meeting URL not found for doctor.")
        return

    # Get email addresses of patient and doctor
    patient_email = fetch_data("SELECT Email FROM patient_info WHERE Name = %s", (patient_name,))
    doctor_email = fetch_data("SELECT Email FROM doctor_info WHERE Name = %s", (doctor_name,))

    if patient_email and doctor_email:
        patient_email, doctor_email = patient_email[0][0], doctor_email[0][0]
    else:
        messagebox.showerror("Error", "Email not found for the given patient or doctor name.")
        return

    # Send email to patient
    send_email(patient_email, patient_meeting_url, doctor_name, "patient")

    # Send email to doctor
    send_email(doctor_email, doctor_meeting_url, patient_name, "doctor")

def send_email(receiver_email, meeting_url, name, role):
    ...
    message['Subject'] = 'Video Call Meeting Link'

    body = f"Dear {name},\n\n"
    body += f"Here is the link for your video call meeting.\n"
    body += f"Meeting URL: {meeting_url}\n\n"
    body += "Regards,\nYour eHealthcare Team"
    ...
    try:
        ...
        messagebox.showinfo("Success", f'Mail sent successfully to {role}!')

    except smtplib.SMTPException as e:
        messagebox.showerror("Error", f'Error sending email to {role}: {e}')
...
def create_appointment_frame(root, patient_name):
    ...
    status_label = ttk.Label(frame, text="Appointment Mode:")
